...
const inject_docker_scripts_package = (generator: ImmutableGenerator) => {
  ...
  const content = `\n"APP MAINTENANCE DOCKERIZED": "echo 'APP MAINTENANCE'",
    "d.deps": "m='Fetching Dependencies (** Docker **) ...' && echo $m && docker compose -f docker/compose.yaml run --rm dev mix deps.get",
    "d.comp": "m='Compiling Apps (** Docker **) ...' && echo $m && docker compose -f docker/compose.yaml run --rm dev mix compile",
    "d.mig": "m='Running Database Migrations (** Docker **) ...' && echo $m && docker compose -f docker/compose.yaml run --rm dev mix ecto.migrate",
    ...
    "d.serv": "m='Starting Server (** Docker **) ...' && echo $m && docker compose -f docker/compose.yaml up dev",
    ...
    "d.hold": "m='Holding Containers (** Docker **) ...' && echo $m && yarn d.up && docker compose -f docker/compose.yaml exec dev sleep 1000000",
